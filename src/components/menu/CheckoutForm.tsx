...
export default function CheckoutForm({ open, onClose, cart, total, tableNumber, onSuccess }: Props) {
  ...
  const handleContinue = () => {
    ...
    if (step === 2) {
      if (!phone.trim()) {
        setError("Ingresa tu mesa o telefono");
        return;
      }
      if (!tableNumber && !isValidPhone) {
  ...
  const displayMethods = paymentMethods.length > 0
    ? paymentMethods
    : [{ id: 0, name: "Efectivo", enabled: 1, details: [] }];
            ...
            <div className="p-6">
              <div className="border-2 border-dashed border-[var(--brand-primary)]/30 rounded-2xl p-5 bg-[var(--brand-bg)]">
                <div className="text-center mb-4">
                  <div className="w-12 h-12 rounded-xl bg-[var(--brand-primary)] flex items-center justify-center text-white font-bold text-lg mx-auto mb-2">
                    L
                  </div>
                ...
                <div className="border-t border-dashed border-[var(--brand-border)] pt-3 space-y-1">
                  <div className="flex justify-between text-[10px] text-[var(--brand-text-muted)]">
                    <span>Subtotal</span>
                    <span>${ticket.total.toFixed(0)}</span>
                  </div>
                  <div className="flex justify-between items-center pt-1">
                    <span className="text-xs font-bold text-[var(--brand-text)]">TOTAL</span>
                    <span className="text-xl font-bold text-[var(--brand-primary)]">${ticket.total.toFixed(0)}</span>
                  </div>
                  <div className="flex justify-between text-[10px] text-[var(--brand-text-muted)]">
                ...
                {ticket.notes && (
                  <div className="border-t border-dashed border-[var(--brand-border)] mt-3 pt-3">
                    <p className="text-[10px] text-[var(--brand-text-muted)] uppercase mb-0.5 font-semibold">Notas</p>
                    <p className="text-[11px] text-[var(--brand-text-secondary)] italic">{ticket.notes}</p>
                  </div>
                )}
              </div>

              <button onClick={onClose}
                className="w-full mt-4 py-3 bg-[var(--brand-primary)] text-white text-sm font-semibold rounded-xl hover:bg-[var(--brand-primary-dark)] transition-all">
                Cerrar
              </button>
            </div>
          ) : (
            <>
              ...
              <div className="p-5 border-b border-[var(--brand-border)] flex items-center justify-between">
                <div>
                  ...
                  <div className="flex items-center gap-1 mt-1">
                    {[1, 2, 3].map((s) => (
                      <div key={s} className={`w-10 h-1 rounded-full transition-all ${s <= step ? "bg-[var(--brand-primary)]" : "bg-gray-200"}`} />
                    ))}
                  </div>
                </div>
              ...
              {step === 1 && (
                <div className="p-5 space-y-3">
                  ...
                  <button onClick={handleContinue} className="btn-primary w-full py-3 text-base font-semibold">
                    Continuar →
                  </button>
                </div>
              )}
              ...
              {step === 2 && (
                <div className="p-5 space-y-4">
                  <div>
                    <label className="text-xs font-semibold text-[var(--brand-text-secondary)] mb-1.5 block uppercase tracking-wider">
                      Como te llamamos?
                    </label>
                  ...
                  <div>
                    ...
                    <input type="tel" value={phone} onChange={(e) => setPhone(e.target.value.replace(/\D/g, ""))} required
                      ...
                      className={`w-full px-4 py-3 rounded-xl border text-sm bg-white focus:outline-none focus:ring-2 transition-all ${
                        !tableNumber && phone.length > 0 && !isValidPhone
                          ? "border-red-300 focus:ring-red-200 focus:border-red-400"
                          : "border-[var(--brand-border)] focus:ring-[var(--brand-primary)]/20 focus:border-[var(--brand-primary)]"
                      }`}
                      placeholder="10 digitos: 5512345678" />
                    {!tableNumber && (
                      <p className={`text-[10px] mt-1 ${phone.length > 0 && !isValidPhone ? "text-red-400" : "text-[var(--brand-text-muted)]"}`}>
                        {phone.length === 0
                          ? "Debe tener exactamente 10 digitos"
                          : !isValidPhone
                            ? `Faltan ${10 - digitsOnly.length} digitos`
                            : "Numero valido"}
                      </p>
                    )}
                  </div>
                  ...
                  <div>
                    ...
                    <div className={`grid gap-2 ${displayMethods.length === 2 ? "grid-cols-2" : "grid-cols-3"}`}>
                        ...
                        <button key={m.id} type="button"
                          ...
                          className={`py-2.5 px-2 text-xs rounded-xl border font-medium transition-all ${
                            payment === m.name
                              ...
                              : "border-[var(--brand-border)] text-[var(--brand-text-secondary)] hover:border-[var(--brand-primary-light)] bg-white"
                          }`}>
                          {m.name}
                        </button>
                      ))}
                  ...
                  <div>
                    ...
                    <textarea value={notes} onChange={(e) => setNotes(e.target.value)} rows={2}
                      className="w-full px-4 py-3 rounded-xl border border-[var(--brand-border)] text-sm bg-white focus:outline-none focus:ring-2 focus:ring-[var(--brand-primary)]/20 focus:border-[var(--brand-primary)] resize-none transition-all"
                      placeholder="Ej: sin azucar, extra shot, sin hielo..." />
                  </div>
