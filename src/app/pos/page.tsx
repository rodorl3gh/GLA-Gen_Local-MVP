...
export default function PosPage() {
  ...
  const fetchProducts = useCallback(async () => {
    try {
      const res = await fetch("/api/catalog"); const data = await res.json();
  ...
  useEffect(() => {
    fetch("/api/promotions").then(r => r.json()).then(d => { if (Array.isArray(d)) setPromotions(d); }).catch(() => {});
  }, []);
  useEffect(() => {
    const hs = (e: StorageEvent) => { if (e.key === FAVORITES_KEY) setFavorites(loadFavorites()); if (e.key === CART_KEY) setCart(loadCart()); if (e.key === GRID_COLS_KEY) setGridCols(loadGridCols()); };
    window.addEventListener("storage", hs); return () => window.removeEventListener("storage", hs);
  }, []);
  useEffect(() => { const i = setInterval(fetchProducts, 30000); return () => clearInterval(i); }, [fetchProducts]);
  ...
  const isFavorite = (id: number, type: "product" | "promo"): boolean => favorites.some(f => f.id === id && f.type === type);

  const handleSelectProduct = (product: Product) => { setSelectedProduct(product); setEditIndex(null); setShowCustomize(true); };
  ...
  const handleEdit = (index: number) => {
    ...
    const prod = products.find(p => p.id === item.id); if (!prod) return;
    setSelectedProduct(prod); setEditIndex(index); setShowCustomize(true);
  };
  const updateQuantity = (index: number, delta: number) => {
    setCart(prev => { const u = prev.map((item, i) => i === index ? { ...item, quantity: Math.max(0, item.quantity + delta) } : item); return u.filter(item => item.quantity > 0); });
  };
  const removeItem = (index: number) => setCart(prev => prev.filter((_, i) => i !== index));
  const resetCart = () => { if (cart.length > 0) setCart([]); };

  const openPromoModal = (promo: Promotion) => { setSelectedPromo(promo); setPromoSelections({}); setPromoSlotSelections({}); setShowPromoModal(true); };
  const addPromoToCart = () => {
    if (!selectedPromo) return; const cfg = selectedPromo.config || {}; let subItems: { name: string; productId: number }[] = [];
    if (selectedPromo.type === "promotion") {
      const pickCount = cfg.pickCount || 2;
      const selected = Object.entries(promoSelections).filter(([_, q]) => q > 0).map(([pid, q]) => { const p = products.find(pr => pr.id === Number(pid)); return { name: p ? p.name : `Producto #${pid}`, productId: Number(pid), quantity: q }; });
      if (selected.reduce((s, i) => s + i.quantity, 0) < pickCount) return;
      ...
      if (!slots.every((slot: any, idx: number) => { const sel = promoSlotSelections[idx]; return sel !== undefined && sel > 0; })) return;
      slots.forEach((slot: any, idx: number) => { const pid = promoSlotSelections[idx]; if (pid) { const p = products.find(pr => pr.id === pid); subItems.push({ name: p ? p.name : slot.label, productId: pid }); } });
    }
  ...
  const { favProducts, favPromos, restProducts, hasFavorites } = useMemo(() => {
    const fp: Product[] = [];
    const fpr: Promotion[] = [];
    const favIds = new Set(favorites.map(f => `${f.type}:${f.id}`));
    for (const f of favorites) {
      if (f.type === "product") { const p = products.find(pr => pr.id === f.id); if (p) fp.push(p); }
      else { const pr = promotions.find(pr => pr.id === f.id); if (pr) fpr.push(pr); }
    }
    const rp = filteredProducts.filter(p => !favIds.has(`product:${p.id}`));
  ...
  return (
    <div className="h-screen flex flex-col bg-[var(--brand-bg)] overflow-hidden">
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto p-4 sm:p-6">
                  ...
                  {favPromos.map(promo => (
                    <div key={`fav-r-${promo.id}`} className="relative">
                      <button onClick={() => openPromoModal(promo)}
                        className="w-full text-left glass-card overflow-hidden hover:shadow-md transition-all duration-150 hover:-translate-y-0.5 active:scale-[0.98] ring-2 ring-yellow-300">
                        <div className="aspect-[4/3] bg-[var(--brand-bg)] overflow-hidden relative">
                          {promo.image_path ? (
                            <img src={getImageUrl(promo.image_path) || ""} alt={promo.name} className="w-full h-full object-cover" loading="lazy" />
                          ) : (
                            <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-yellow-100 to-yellow-50">
                              <svg className="w-5 h-5 text-yellow-400" viewBox="0 0 24 24" fill="currentColor"><path d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.783-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z" /></svg>
                            </div>
                          )}
                          <span className={`absolute top-1 left-1 text-[7px] px-1 py-0.5 rounded-full font-medium ${promo.type === "promotion" ? "bg-amber-100 text-amber-700" : "bg-sky-100 text-sky-700"}`}>
                      ...
                      <button onClick={(e) => { e.stopPropagation(); toggleFavorite(promo.id, "promo"); }}
            ...
            {filteredProducts.length === 0 && (
              <div className="text-center py-12"><p className="text-sm text-[var(--brand-text-muted)]">No hay productos en esta categoria</p></div>
            )}
                  ...
                  {promotions.filter(p => !isFavorite(p.id, "promo")).map(promo => (
                    <div key={promo.id} className="relative">
                      <button onClick={() => openPromoModal(promo)}
                        ...
                        <div className="aspect-[4/3] bg-[var(--brand-bg)] overflow-hidden relative">
                          {promo.image_path ? (
                            <img src={getImageUrl(promo.image_path) || ""} alt={promo.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" loading="lazy" />
                          ) : (
                            <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-[var(--brand-primary)]/5 to-[var(--brand-primary)]/10">
                              <svg className="w-8 h-8 text-[var(--brand-primary)]/20" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1} d="M12 8v13m0-13V6a2 2 0 112 2h-2zm0 0V5.5A2.5 2.5 0 109.5 8H12zm-7 4h14M5 12a2 2 0 110-4h14a2 2 0 110 4M5 12v7a2 2 0 002 2h10a2 2 0 002-2v-7" /></svg>
                            </div>
                          )}
                          <span className={`absolute top-1 left-1 text-[7px] px-1 py-0.5 rounded-full font-medium ${promo.type === "promotion" ? "bg-amber-100 text-amber-700" : "bg-sky-100 text-sky-700"}`}>
                            {promo.type === "promotion" ? "Promo" : "Paquete"}
                      ...
                      <button onClick={(e) => { e.stopPropagation(); toggleFavorite(promo.id, "promo"); }}
                        ...
                        <svg className={`w-3.5 h-3.5 ${isFavorite(promo.id, "promo") ? "text-yellow-500 fill-yellow-500" : "text-gray-400"}`} viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.783-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z" /></svg>
                      </button>
                    </div>
                  ))}
                </div>
      ...
      <CheckoutModal open={showCheckout} onClose={() => setShowCheckout(false)} cart={cart} total={cartTotal}
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl animate-scale-in max-h-[90vh] overflow-y-auto">
              <div className="p-4 border-b border-[var(--brand-border)] flex items-center justify-between">
                <div><h2 className="text-sm font-semibold text-[var(--brand-text)]">{selectedPromo.name}</h2><p className="text-[10px] text-[var(--brand-text-muted)]">{selectedPromo.type === "promotion" ? "Selecciona tus productos" : "Elige una opcion por categoria"}</p></div>
                <button onClick={() => setShowPromoModal(false)} className="p-1.5 rounded-xl hover:bg-gray-50"><svg className="w-5 h-5 text-[var(--brand-text-secondary)]" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg></button>
              </div>
              <div className="p-4 space-y-4">
                {selectedPromo.type === "promotion" && (() => {
                  const cfg = selectedPromo.config || {}; const pickCount = cfg.pickCount || 2; const payCount = cfg.payCount || 1;
                  const eligibleIds: number[] = cfg.eligibleProductIds || []; const picked = Object.values(promoSelections).reduce((s, q) => s + q, 0);
                  return (<><p className="text-xs text-[var(--brand-text-secondary)]">Elige <strong>{pickCount}</strong> productos y paga solo <strong>{payCount}</strong>{picked > 0 && <span className="text-[var(--brand-primary)]"> — {picked} de {pickCount} seleccionados</span>}</p>
